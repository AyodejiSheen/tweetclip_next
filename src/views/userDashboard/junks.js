            // const navigate = useNavigate();
    // let { isDark } = useContext(UiContext)
    // let { getSingleArtboard, artboardLoading, artboardProps, singleArtboard } = useContext(ArtBoardContext);

    // useEffect(() => {
    //     getSingleArtboard(id)
    // }, [id]);


    // const handleDownloadImage = async () => {
    //     const element = document.getElementById("app"),
    //         canvas = await html2canvas(element, { allowTaint: true, useCORS: true }),
    //         // stream = canvas.captureStream(25),
    //         data = canvas.toDataURL("image/jpg"),
    //         link = document.createElement("a");
    //     link.href = data;
    //     link.download = "downloaded-image.";
    //     document.body.appendChild(link);
    //     link.click();
    //     document.body.removeChild(link);
    //     console.log(data);
    // };

        
        
        {/* <div className="mb-6 lg:my-0 lg:hidden">
                <button onClick={() => navigate(-1)} className="font-semibold bg-slate-300 px-6 py-2 rounded-full text-xs md:text-sm"> <i class="lni lni-arrow-left"></i> Back</button>
            </div> */}

            {/* <section id="app" className="bg-blue-500 py-8 md:py-16 2xl:py-24 rounded-lg">
                {
                    artboardLoading ?
                        <section className="flex flex-col justify-between p-6 md:p-14 h-max gap-12 md:gap-24 rounded-xl shadow-2xl w-5/6 2xl:w-2/3 mx-auto" style={{ backgroundColor: `${artboardProps.bgColor}`, color: `${artboardProps.fontColor}`, backgroundImage: `url(${artboardProps.bgUrl})`, backgroundSize: 'cover', backgroundRepeat: 'no-repeat' }}>
                            top */}
            {/* <div className="flex justify-between items-center">
                                <div className="flex gap-3 md:gap-5 items-center">
                                    <div>
                                        <img src={singleArtboard.profileImageUrl} alt="img" className="w-10 md:w-20 xl:w-24 rounded-full" />
                                    </div>

                                    <div>
                                        <h4 className="font-bold text-base md:text-lg xl:text-xl">{singleArtboard.name}</h4>
                                        <p className="font-medium text-slate-600  text-xs md:text-base xl:text-lg" style={{ color: `${artboardProps.fontColor}` }}>@{singleArtboard.username}</p>
                                    </div>
                                </div>
                            </div> */}

            {/* main text */}
            {/* <div >
                <h1 className="w-11/12 break-words animate__animated animate__bounce animate__infinite" style={{ fontFamily: ` ${artboardProps.fontStyle}`, fontSize: `${artboardProps.fontSize}px` }} >
                    {singleArtboard.text}
                </h1>
            </div> */}


            {/* bottom */}
            {/* <div className="space-y-3">
                                <p className="text-xs md:text-base xl:text-lg text-slate-600" style={{ color: `${artboardProps.fontColor}` }}>11:40 AM · Nov 16, 2022 · {singleArtboard.source}</p>
                                <div className="border-y p-2.5 md:p-4">
                                    <div className="text-[10.5px] md:text-base lg:text-sm xl:text-base flex justify-between">
                                        <div className="flex gap-0.5 md:gap-2 text-slate-600" style={{ color: `${artboardProps.fontColor}` }}>
                                            <p className="font-bold">{singleArtboard.retweetCount}</p>
                                            <p>Retweets</p>
                                        </div>
                                        <div className="flex gap-0.5 md:gap-2 text-slate-600" style={{ color: `${artboardProps.fontColor}` }}>
                                            <p className="font-bold">{singleArtboard.quoteCount}</p>
                                            <p>Quote Tweets</p>
                                        </div>
                                        <div className="flex gap-0.5 md:gap-2 text-slate-600" style={{ color: `${artboardProps.fontColor}` }}>
                                            <p className="font-bold">{singleArtboard.likeCount}</p>
                                            <p>Likes</p>
                                        </div>
                                    </div>
                                </div>
                            </div> */}
            {/* 
                            <button onClick={() => handleDownloadImage()}>ddd</button>

                        </section> :
                        <div className=""> */}
            {/* {
                                [1].map((n) => <ProjectSkeleton key={n} theme={isDark === "light" ? "light" : "dark"} />)
                            }
                        </div> */}

            {/* }
            </section> */}